...
import { PaymentMethodRecord } from '../../../services/config/config.models';
...
export class ConfigPaymentMethodsComponent implements OnInit, OnDestroy {
  private readonly service = inject(ConfigPaymentMethodService);
  ...
  readonly rows = signal<PaymentMethodRecord[]>([]);
  readonly total = signal(0);
  readonly loading = signal(false);
  readonly saving = signal(false);
  readonly error = signal<string | null>(null);
  readonly isModalVisible = signal(false);
  readonly editingRow = signal<PaymentMethodRecord | null>(null);
  ...
  readonly columns: AppDataTableColumn<PaymentMethodRecord>[] = [
  ...
  openEdit(row: PaymentMethodRecord): void {
  ...
  handleAction(event: { type: 'edit' | 'delete' | 'custom'; row: PaymentMethodRecord }): void {
    if (event.type === 'edit') {
  ...
  private confirmDelete(row: PaymentMethodRecord): void {
    this.modal.confirm({
      nzTitle: '¿Eliminar método de pago?',
      nzContent: `¿Deseas eliminar ${row.name}?`,
      nzOkDanger: true,
      nzOnOk: () =>
