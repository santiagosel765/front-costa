...
import { ProviderPayload, ProviderRecord, ProvidersApi } from '../../api/providers.api';
...
export class ProviderShellComponent implements OnInit, OnDestroy {
  ...
  readonly rows = signal<ProviderRecord[]>([]);
  ...
  readonly columns: AppDataTableColumn<ProviderRecord>[] = [
    { key: 'name', title: 'Nombre', sortable: true },
    { key: 'taxId', title: 'RUC/NIT', sortable: true, valueGetter: (row) => row.taxId ?? '-' },
    ...
    { key: 'phone', title: 'Teléfono', valueGetter: (row) => row.phone ?? '-' },
    {
      ...
      actions: [
        ...
        { type: 'delete', label: 'Desactivar', icon: 'stop', danger: true, confirmTitle: '¿Desactivar proveedor?' },
      ],
    },
  ];
  ...
  handleAction(event: { type: 'edit' | 'delete' | 'custom'; row: ProviderRecord }): void {
    if (event.type === 'edit') {
  ...
  openCreate(): void {
    ...
    this.form.reset({ name: '', taxId: '', email: '', phone: '' });
  ...
  save(): void {
    ...
    const payload = this.form.getRawValue() as ProviderPayload;
    const currentId = this.editingId();
    this.loading.set(true);

    const request$ = currentId
      ? this.providersApi.update(currentId, payload)
      : this.providersApi.create(payload);

    request$.subscribe({
      next: () => {
        this.message.success(currentId ? 'Proveedor actualizado' : 'Proveedor creado');
        this.isModalVisible.set(false);
  ...
  private openEdit(row: ProviderRecord): void {
  ...
  private remove(id: string): void {
    this.loading.set(true);
    this.providersApi.delete(id).subscribe({
      next: () => {
        this.message.success('Proveedor desactivado');
        this.load(this.tableState.snapshot);
      },
      error: () => {
        this.loading.set(false);
