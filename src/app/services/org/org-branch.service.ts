...
export class OrgBranchService {
  ...
  list(query: CatalogQuery): Observable<PagedResponse<CatalogRecord>> {
    return this.api
      .get<ApiResponse<PagedResponse<CatalogRecord>> | PagedResponse<CatalogRecord>>('/v1/org/branches', {
  ...
  create(dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .post<ApiResponse<CatalogRecord> | CatalogRecord>('/v1/org/branches', dto)
      .pipe(map((response) => unwrapApiResponse(response)));
  }

  update(id: string, dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .put<ApiResponse<CatalogRecord> | CatalogRecord>(`/v1/org/branches/${id}`, dto)
      .pipe(map((response) => unwrapApiResponse(response)));
  }
