...
export class ConfigParameterService {
  ...
  list(query: CatalogQuery): Observable<PagedResponse<CatalogRecord>> {
    return this.api
      .get<ApiResponse<PagedResponse<CatalogRecord>> | PagedResponse<CatalogRecord>>('/v1/config/parameters', {
  ...
  create(dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .post<ApiResponse<CatalogRecord> | CatalogRecord>('/v1/config/parameters', dto)
  ...
  update(id: string, dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .put<ApiResponse<CatalogRecord> | CatalogRecord>(`/v1/config/parameters/${id}`, dto)
      .pipe(map((response) => normalizeCatalogRecord(unwrapApiResponse(response))));
  }
