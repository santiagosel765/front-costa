...
export class ConfigCurrencyService {
  ...
  list(query: CatalogQuery): Observable<PagedResponse<CatalogRecord>> {
    return this.api
      .get<ApiResponse<PagedResponse<CatalogRecord>> | PagedResponse<CatalogRecord>>('/v1/config/currencies', {
  ...
  create(dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .post<ApiResponse<CatalogRecord> | CatalogRecord>('/v1/config/currencies', dto)
  ...
  update(id: string, dto: CatalogDto): Observable<CatalogRecord> {
    return this.api
      .put<ApiResponse<CatalogRecord> | CatalogRecord>(`/v1/config/currencies/${id}`, dto)
      .pipe(map((response) => normalizeCatalogRecord(unwrapApiResponse(response))));
  }
